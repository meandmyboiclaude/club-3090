#!/bin/bash
# endgame8020-boot-guard.sh — wired as ExecStartPost on vllm-qwen36-endgame.service.
#
...
COMPOSE_FILE=endgame8020.yml
WAIT_S=${ENDGAME_BOOT_GUARD_WAIT_S:-900}

log() { echo "[endgame8020-boot-guard] $*"; }
...
case "$outcome" in
  ...
  restarted)
    ...
    ts=$(date +%Y%m%d-%H%M%S)
    podman logs "$C" > "/home/user/shared/endgame8020-bootguard-recycle-$ts-container.log" 2>&1 \
