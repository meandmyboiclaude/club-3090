...
def _make_gm_patcher() -> TextPatcher | None:
    ...
    # `quantization/gptq_marlin.py`; the GPTQMarlin create_weights body
    # (with the scales_and_zp_size floor-div + qweight/g_idx/scales/qzeros
    # register block) moved verbatim into `quantization/auto_gptq.py`. All
    # three anchors are byte-identical at the new path (auto_gptq.py L373/
    # L378/L442-445); #39460 still NOT merged (floor-div live, no cdiv).
    # Fallback to the old path for pre-split trees.
    target = (
        resolve_vllm_file(
...
def apply() -> tuple[str, str]:
    ...
    gm = _make_gm_patcher()
    if gm is None:
        return "skipped", "auto_gptq.py / gptq_marlin.py not found"
