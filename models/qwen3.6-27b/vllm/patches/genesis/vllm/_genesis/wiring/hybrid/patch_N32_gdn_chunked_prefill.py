...
# ─── Anchor: prefill branch in _forward_core ────────────────────────
# Re-anchored 2026-06-13 for the nightly image where
# `gdn_linear_attn.py` moved to `gdn/qwen_gdn_linear_attn.py` and the
# prefill branch was rewritten with new precomputed metadata
# (prefill_state_indices / prefill_has_initial_state /
# prefill_query_start_loc; no .contiguous(); label "# 2.3"; a
# split_non_spec decode-stitch now follows the cache write).
# Matches the EXACT prefill `if attn_metadata.num_prefills > 0:`
# branch (through the ssm_state cache write) of
# `model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py:`
# `QwenGatedDeltaNetAttention._forward_core`. The trailing
# split_non_spec stitch is intentionally left OUTSIDE the anchor —
# PN32 only rewrites the FLA call + cache write; the stitch operates
# on the resulting core_attn_out_non_spec afterward (orthogonal).
...
PN32_REPLACEMENT = (
    ...
    "\n"
    "            # Single-sequence detection: the prefill cu_seqlens is the\n"
    "            # metadata-builder's rebased prefill-only start_loc (decodes\n"
    "            # already peeled off and offset-subtracted). Shape [2] = one\n"
    "            # [0, T] entry. Multi-seq has shape [N+1] for N>1.\n"
    "            _genesis_pn32_prefill_qsl = attn_metadata.prefill_query_start_loc\n"
    "            _genesis_pn32_T_full = int(query_non_spec.shape[1])\n"
    "            _genesis_pn32_is_single_seq = (\n"
    "                _genesis_pn32_prefill_qsl is not None\n"
    "                and _genesis_pn32_prefill_qsl.shape[0] == 2\n"
    "            )\n"
    ...
    "                        dtype=_genesis_pn32_prefill_qsl.dtype,\n"
    "                    )\n"
