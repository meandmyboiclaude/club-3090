...
limits any prefill chunk to this value (verified unchanged on vllm-new
nightly 9c7f7741 at `vllm/v1/core/sched/scheduler.py:408-409`; was 390-391
on the old aa2b56ffb build — line drift only, mechanism byte-identical):
...
vllm-new (nightly 9c7f7741) rebase check — `SchedulerConfig.__post_init__`
semantics verified unchanged at config/scheduler.py:224-255:
  * The P74 anchor (`if self.enable_chunked_prefill:` info_once block
    immediately followed by `if self.max_num_partial_prefills > 1:`) matches
    byte-for-byte (scheduler.py:238-244).
  * The upstream auto-threshold `long_prefill_token_threshold =
    int(max_model_len * 0.04)` fires only inside the
    `max_num_partial_prefills > 1` branch (scheduler.py:245-246) — IDENTICAL
    in old aa2b56ffb. P74 injects BEFORE that branch, so for our prod config
    (single partial prefill) P74 governs; for multi-partial-prefill it
    composes (P74 sets threshold first → upstream's `== 0` guard is False →
    no double-set). No regression.
  * Runtime honoring intact at scheduler.py:408-409 (the clamp
