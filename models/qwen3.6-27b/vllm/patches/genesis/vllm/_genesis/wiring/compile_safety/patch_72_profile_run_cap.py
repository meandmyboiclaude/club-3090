...
vllm-new (nightly 9c7f7741) rebase check
-----------------------------------------
- Anchor verified byte-for-byte at v1/worker/gpu_model_runner.py:6227-6230
  (the "Add is_profile here ..." comment + `self._dummy_run(
  self.max_num_tokens, is_profile=True)`); was line 6165 on old aa2b56ffb —
  line drift only.
- `_dummy_run` signature unchanged enough: positional `num_tokens` is still
  the first param and `is_profile` is still a kwarg
  (gpu_model_runner.py:5599-5613), so passing a capped int positionally +
  `is_profile=True` remains valid.
- Breakable-CUDA-graphs (#44050) and EAGLE graph-pool removals
  (#44078/#44338) do NOT invalidate this cap: profile_run drives a
  `CUDAGraphMode.NONE` warmup (no capture), and the failure this guards is a
  Dynamo *fake-tensor symbolic-shape* reconciliation inside
  moe_align_block_size — orthogonal to any CUDA graph memory-pool plumbing.
  The cap shrinks the traced M; it never touches graph capture/pool logic.
