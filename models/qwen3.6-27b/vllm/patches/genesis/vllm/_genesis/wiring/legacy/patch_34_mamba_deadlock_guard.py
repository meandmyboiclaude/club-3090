...
# partial prefix-cache) rewrote `_mamba_block_aligned_split` — the branch is
# now an `elif`, and the alignment floors the chunk END instead of the
# length: `aligned_end = after_sched // bs * bs; num_new_tokens =
# max(aligned_end - num_computed_tokens, 0)`. The zero-collapse is NOT
# absorbed: when the encoder-cache gap is < block_size the new expression
# still yields 0 and the request is skipped every step (the upstream
# comment even documents "May yield 0 ... the caller then skips the
# request"). Guard re-anchored to the new form: compute upstream's aligned
# length, apply it only when > 0, else keep the sub-block value (state
# "simply not cached", as before). NOT dev799-compatible (framework
# supports a single anchor form per sub-patch).
# REBASE 2026-07-14 (dev1060cherry): upstream replaced the inline
# `aligned_end = ... // block_size * block_size` with the `round_down()`
# helper wrapped in `max(..., 0)`. The zero-collapse is UNCHANGED — when the
# encoder-cache gap is < block_size the expression still yields 0 and the
# request is skipped every step. Re-anchored to the new round_down form;
# same guard (apply the aligned length only when > 0, else keep sub-block).
_OLD = (
...
_NEW = (
    ...
    "                _genesis_p34_aligned = (\n"
    "                    round_down(num_computed_tokens_after_sched, block_size)\n"
    "                    - num_computed_tokens\n"
    "                )\n"
    "                if _genesis_p34_aligned > 0:\n"
    "                    num_new_tokens = _genesis_p34_aligned"
)
