...
================================================================
STATUS ON vllm-new (nightly 9c7f7741): EFFECTIVELY OBSOLETE — SELF-SKIPS
================================================================

P78 is DISABLED in prod (`GENESIS_ENABLE_P78_TOLIST_CAPTURE_GUARD=0`,
`default_on=False`) so `should_apply("P78")` returns False and apply()
self-skips at the dispatcher gate — it never touches the file at boot.

Independently, the Sites A/C/D/E `.tolist()` problem this patch targeted
has been ABSORBED by the installed TurboQuant backend itself. The Genesis-
authored `v1/attention/backends/turboquant_attn.py` shipped in this stack
was rewritten to thread CPU-resident mirrors through the metadata and
compute the prefill max WITHOUT a GPU→CPU sync, which is exactly the
architectural fix P78 Sites C/D/E proposed. Evidence in vllm-new (identical
to old aa2b56ffb — `diff -q` reports IDENTICAL, so this absorption predates
the rebase):
  * TurboQuantMetadata already carries `query_start_loc_cpu` / `seq_lens_cpu`
    fields (turboquant_attn.py:190-193) — Site C is redundant.
  * Builder.build() already populates them from
    `cam.query_start_loc_cpu` / `cam.seq_lens_cpu_upper_bound`
    (turboquant_attn.py:237-238) — Site D is redundant, and the Site D
    anchor no longer matches (the return block has extra `_cpu` kwargs).
  * forward() now does `prefill_max_seq = int(
    attn_metadata.seq_lens_cpu[num_decodes:].max())` (turboquant_attn.py:487),
    NOT the old `max(seq_lens[num_decodes:].tolist())` — Site A/E anchor is
    GONE (0 matches). The illegal-under-capture sync no longer exists.
  * _prefill_attention() (Site B) still has `.tolist()` calls but they are
    now guarded by `if query_start_loc_cpu is not None` /
    `if seq_lens_cpu is not None` (turboquant_attn.py:603-610) and the
    builder always populates those CPU mirrors, so the GPU `.tolist()` is
    dead in steady state.

Net: even if an operator force-enables P78=1, Sites D and E (required=True)
fail their anchor check → TextPatcher returns required_anchor_missing →
SKIPPED, no write, no crash. To make apply() report the honest
"upstream/backend absorbed this fix" instead of a generic anchor-drift skip,
the absorbing line `prefill_max_seq = int(attn_metadata.seq_lens_cpu[...]
.max())` is registered in `upstream_drift_markers` below — its presence trips
the Layer-3 upstream-merged short-circuit.
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        ...
        upstream_drift_markers=[
            ...
            # [Genesis P78 rebase 9c7f7741] The installed TurboQuant backend
            # already computes prefill_max_seq from the CPU mirror with no
            # GPU→CPU sync — exactly the Sites A/C/D/E fix. Its presence means
            # P78 is obsolete; trip Layer-3 "upstream_merged" so apply() reports
            # honestly instead of a generic anchor-drift skip. (Site B's
            # anchor still matches but is also CPU-guarded; reporting absorbed
            # for the whole patch is correct.)
