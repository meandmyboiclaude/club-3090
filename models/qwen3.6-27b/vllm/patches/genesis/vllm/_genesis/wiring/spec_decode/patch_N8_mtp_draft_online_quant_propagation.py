...
# REBASE 2026-07-07 (nightly-69715823): upstream moved the QuantizationConfig
# import under `if TYPE_CHECKING:` (utils.py) — re-anchored to that block; the
# injected try/except import stays at module level (runtime), NOT inside the
# TYPE_CHECKING guard.
# REBASE 2026-07-13 (nightly-9e57de71, dev1060): upstream inserted
# `from transformers import PretrainedConfig` + `WeightRenaming` between
# `if TYPE_CHECKING:` and the QuantizationConfig line — anchor narrowed to
# the QuantizationConfig import line itself (last line of the block on BOTH
# dev799 and dev1060, so this anchor is backward-compatible). The injected
# block is column-0, which terminates the TYPE_CHECKING suite — still
# module-level runtime code.
PN8_IMPORTS_OLD = (
