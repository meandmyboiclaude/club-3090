...
# Re-anchored for dev491 (nightly-1033ffac): upstream changed the placeholder
# increment from `+= 1 + cur_num_spec_tokens` to a multi-line
# `+= (self.num_sampled_tokens_per_step + cur_num_spec_tokens)`. Anchor updated
# to match; the P58 replacement preserves the new increment and swaps only the
# `spec_token_ids = self._spec_token_placeholders` assignment for the counter.
ASYNC_SCHED_OLD = (
