...
(line refs are vllm-new 2026-06-07 — both anchor blocks verified
byte-identical to OLD, function moved up ~25 lines but text unchanged):

  - Line 1236-1237 (fp8_w8a8 block-quant path): `num_warps=4, num_stages=3
    if not ROCm else 2`.
  - Line 1282-1298 (bf16/fp16/fp8-per-tensor path): `num_warps=4 if M<=128
    else 8`, `num_stages=4 if M<=32 else 3` (CUDA).

  NOTE: the `int4_w4a16`/`int8_w8a16` branch (lines 1239-1254) — the path
  Qwen3.6-27B AutoRound W4A16 takes on the Triton fallback — sets ONLY
  BLOCK_SIZE_M/GROUP_SIZE_M/SPLIT_K and does NOT set num_warps/num_stages,
  so this overlay never touches it. PR #43731's Triton W4A16 CUDA fallback
  is a *dense Linear* kernel (kernels/linear/) and does not affect this
  MoE config path.
