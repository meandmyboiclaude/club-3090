...
Sister to PN35 (text-only inputs_embeds buffer skip). NOTE: vllm#35975
is **still OPEN** as of the 2026-06-07 nightly (verified via gh api,
state=open, merged=false) — earlier revisions of this docstring claimed
it was merged; that was wrong. PN35's anchors remain live and needed.

⚠ vllm-pin drift (2026-06-07): `language_model_only` moved from
`model_config` onto the nested `multimodal_config`
(config/multimodal.py:77; config/model.py threads it through as an
InitVar). `_is_text_only_mode` below reads `vllm_config.language_model_only`
then falls back to `vllm_config.model_config.language_model_only` —
neither reaches `multimodal_config.language_model_only`, so on this pin
the text-only check returns False and the marker never fires. Because
PN62 is marker-only instrumentation with NO production hook reading
`_pn62_skip_vit_scratch`, this is functionally inert either way: the
wrapper still safely delegates to the original `_dummy_run`. Left as-is
(no behavioural change to fix) until the real ViT-alloc hook lands and
the detection is rewritten to read the nested multimodal_config field.
...
Sister patch: PN35 (text-only inputs_embeds skip, vllm#35975 — still OPEN).
"""
