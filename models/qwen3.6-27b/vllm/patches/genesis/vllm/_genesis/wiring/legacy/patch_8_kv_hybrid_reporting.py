...
================================================================
STATUS ON vllm-new (nightly 9c7f7741): INERT / RETIRED — verified safe
================================================================

The production log reports "P8 retired/skip". Confirmed on rebase: P8
self-skips and does NOT modify any file or crash boot. Root cause is that
upstream effectively ABSORBED P8's intent by REWRITING the per-token
capacity computation, which removed the very anchors P8 targets:

  * `_report_kv_cache_config` no longer uses the under-reporting formula
    `num_blocks // len(kv_cache_groups) * min_block_size`. It now computes
    `num_tokens = int(max_concurrency * max_model_len)` via
    `get_max_concurrency_for_kv_cache_config(...)`, and its own docstring
    states the naive `num_blocks // num_groups * block_size` formula
    "underestimates capacity" for hybrid/SWA models — precisely the bug P8
    fixed (vllm-new v1/core/kv_cache_utils.py:1741-1755). So the
    `_KV_CALLSITE_OLD` anchor (`min_block_size = min(...)`) is GONE
    (0 matches), and the `_KV_IMPORT_OLD`/`_OLD_V2` import anchors no longer
    match (upstream added `HiddenStateCacheSpec` to the import block,
    kv_cache_utils.py:22-35).
  * `scheduler.py` no longer has `self.max_num_kv_tokens` at all
    (0 matches in vllm-new v1/core/sched/scheduler.py) — the
    `_SCHED_CALLSITE_OLD` anchor is GONE. The `_SCHED_IMPORT_OLD` anchor
    still matches (scheduler.py:37-39) but is never reached because the
    kv side skips first (atomic guard below).

This absorption predates the rebase: the OLD running build (aa2b56ffb)
ALSO has the `max_concurrency`-based report and no `max_num_kv_tokens`
(both confirmed by reading vllm-old), which is why P8 was already
"retired" in production before this nightly bump.

Boot-safety trace on vllm-new:
  1. `_patcher_kv()`: required sub-patch `p8_kv_imports` anchor missing →
     TextPatcher returns SKIPPED(required_anchor_missing) WITHOUT writing
     (validate-ALL-before-write, text_patch.py:151-163).
  2. The atomic guard in apply() sees result_kv == SKIPPED (reason !=
     "upstream_merged") → returns ("skipped", ...) and REFUSES to touch
     scheduler.py. No `token_capacity_kv_cache_groups` import is injected,
     so the historical ImportError crash-loop is impossible here.

NOTE on drift markers: we deliberately do NOT add the new `max_concurrency`
report as an `upstream_merged` drift marker. Doing so would flip the kv
result to SKIPPED(reason="upstream_merged"), which the atomic guard treats
as "helper exists upstream → safe to apply scheduler.py" (apply() line
~462-468). That assumption is FALSE here — upstream did NOT add
`token_capacity_kv_cache_groups`; it deleted the formula. Injecting the
scheduler import would then risk an ImportError. The current
required_anchor_missing path is the correct, conservative self-skip; leave
it. (`_make_patcher()` still emits a benign WARNING that neither import
anchor matches — harmless; it does not gate anything.)
