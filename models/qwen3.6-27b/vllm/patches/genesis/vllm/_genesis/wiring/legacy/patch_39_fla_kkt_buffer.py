...
Install model — text-patch + setattr (survives `exec vllm serve`)
----------------------------------------------------------------
Until 2026-07-25 this patch installed ONLY via `setattr()` from `apply()`.
Under the shipped compose entrypoint

    python3 -m vllm._genesis.patches.apply_all   # apply runs in shell process
    exec vllm serve "$@"                          # exec REPLACES the process

`exec` discards that Python state, so P39a logged "[Genesis] applied: P39a"
on every boot and then did nothing where it mattered. Fixed by copying
P103's sanctioned mechanism: a text-patched, module-import-time self-install
hook appended to `chunk_scaled_dot_kkt.py`. Opt-in and DEFAULT OFF behind
`GENESIS_ENABLE_P39A_SELFINSTALL` — see `_ENV_SELFINSTALL` below.

Author: Sandermage(Sander)-Barzov Aleksandr, Ukraine, Odessa
Status: v7.3 implementation (self-install added 2026-07-25)
"""
...
_P39A_SELF_INSTALL_BLOCK = (
    ...
    "# install), the try/except keeps this module importable.\n"
    "try:\n"
    "    import os as _genesis_p39a_os\n"
    "    if _genesis_p39a_os.environ.get(\n"
    "        \"GENESIS_ENABLE_P39A_SELFINSTALL\", \"\"\n"
    "    ).strip().lower() in (\"1\", \"true\", \"yes\", \"on\"):\n"
    "        from vllm._genesis.wiring.legacy.patch_39_fla_kkt_buffer "
    "import (\n"
