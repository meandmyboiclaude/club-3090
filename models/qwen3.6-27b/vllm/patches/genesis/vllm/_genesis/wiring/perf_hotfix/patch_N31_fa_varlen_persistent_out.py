...
PN31_REPLACEMENT_NO_VER = (
    ...
    "                )\n"
    "        else:\n"
    "            # [2026-07-14 BUG-072 ROOT-CAUSE FIX] large prefills: ONE\n"
    "            # grow-only buffer, sliced per call. The old exact-total_q\n"
    "            # dict minted a permanent ~300MB buffer per DISTINCT large\n"
    "            # prefill length (unbounded) -> allocated plateaued ~1.9GiB\n"
    "            # above the profiled budget -> every later large prefill\n"
    "            # OOM-aborted until restart. Grow-only cap: worst case ONE\n"
    "            # max-model-len-sized buffer (~600MB @60K) instead of one\n"
    "            # per length. 4096-multiple rounding limits regrow count.\n"
    "            _genesis_pn31_cap = -(-_genesis_pn31_total_q // 4096) * 4096\n"
    "            _genesis_pn31_big = getattr(self, '_genesis_pn31_big_buf', None)\n"
    "            if (\n"
    "                _genesis_pn31_big is None\n"
    "                or _genesis_pn31_big.shape[0] < _genesis_pn31_cap\n"
    "                or _genesis_pn31_big.shape[1] != _genesis_pn31_n_heads\n"
    "                or _genesis_pn31_big.shape[2] != _genesis_pn31_head_dim\n"
    "                or _genesis_pn31_big.dtype != q.dtype\n"
    "                or _genesis_pn31_big.device != q.device\n"
    "            ):\n"
    "                self._genesis_pn31_big_buf = None  # free before regrow\n"
    "                _genesis_pn31_big = _genesis_pn31_torch.empty(\n"
    "                    (_genesis_pn31_cap, _genesis_pn31_n_heads,\n"
    "                     _genesis_pn31_head_dim),\n"
    "                    dtype=q.dtype, device=q.device,\n"
    "                )\n"
    "                self._genesis_pn31_big_buf = _genesis_pn31_big\n"
