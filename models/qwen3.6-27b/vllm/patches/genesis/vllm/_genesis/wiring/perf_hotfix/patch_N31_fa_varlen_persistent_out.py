...
PN31_REPLACEMENT_NO_VER = (
    ...
    "        if _genesis_pn31_total_q <= 128:\n"
    "            # capture-relevant sizes ONLY (max_cudagraph_capture_size=32,\n"
    "            # margin to 128): exact-key pool — captured graphs bake the\n"
    "            # buffer address, so these must stay per-key stable forever.\n"
    "            # [2026-07-14 BUG-072 v3] was <=4096: every DISTINCT residual\n"
    "            # prefill-chunk length (storm shapes: 479..3909 rows) minted\n"
    "            # a permanent buffer PER LAYER x17 — ~1.3GiB across one storm.\n"
    "            # Uncaptured sizes (>128) are eager -> no address pinning\n"
    "            # needed -> share the grow-only buffer below.\n"
    ...
    "                )\n"
    "        else:\n"
    "            # [2026-07-14 BUG-072 KILL-SHOT v2] large prefills: ONE\n"
    "            # grow-only buffer SHARED ACROSS ALL LAYERS (class-level\n"
    "            # registry), sliced per call. v1 (grow-only, per-instance)\n"
    "            # still minted one buffer PER full-attention layer — 17 x\n"
    "            # [8192,24,256] fp16 = 1.63 GiB resident; memory-history\n"
    "            # traced ALL 17 residents to this exact site — and the\n"
    "            # 4096-multiple rounding doubled 4128-token chunks to 8192\n"
    "            # rows. Layers run sequentially on one stream and the out\n"
    "            # tensor is consumed by o_proj inside the layer, so\n"
    "            # cross-layer sharing is safe (same pattern as P28's shared\n"
    "            # GDN buffer, dev799-proven). 512-multiple rounding:\n"
    "            # 4128 -> 4608 rows (~54 MiB total, was 1.63 GiB).\n"
    "            _genesis_pn31_cap = -(-_genesis_pn31_total_q // 512) * 512\n"
    "            _genesis_pn31_cls = type(self)\n"
    "            _genesis_pn31_reg = getattr(\n"
    "                _genesis_pn31_cls, '_genesis_pn31_shared_big', None)\n"
    "            if _genesis_pn31_reg is None:\n"
    "                _genesis_pn31_reg = {}\n"
    "                _genesis_pn31_cls._genesis_pn31_shared_big = (\n"
    "                    _genesis_pn31_reg\n"
    "                )\n"
    "            _genesis_pn31_key = (\n"
    "                _genesis_pn31_n_heads,\n"
    ...
    "                q.device.index,\n"
    "            )\n"
    "            _genesis_pn31_big = _genesis_pn31_reg.get(_genesis_pn31_key)\n"
    "            if (\n"
    "                _genesis_pn31_big is None\n"
    "                or _genesis_pn31_big.shape[0] < _genesis_pn31_cap\n"
    "            ):\n"
    "                _genesis_pn31_reg[_genesis_pn31_key] = None  # free first\n"
    "                _genesis_pn31_big = _genesis_pn31_torch.empty(\n"
    "                    (_genesis_pn31_cap, _genesis_pn31_n_heads,\n"
    "                     _genesis_pn31_head_dim),\n"
    "                    dtype=q.dtype, device=q.device,\n"
    "                )\n"
    "                _genesis_pn31_reg[_genesis_pn31_key] = _genesis_pn31_big\n"
    "            _genesis_pn31_out = _genesis_pn31_big[:_genesis_pn31_total_q]\n"
    "        if self.fa_version is None:\n"
    "            return flash_attn_varlen_func(\n"
    "                q=q,\n"
    "                k=k,\n"
