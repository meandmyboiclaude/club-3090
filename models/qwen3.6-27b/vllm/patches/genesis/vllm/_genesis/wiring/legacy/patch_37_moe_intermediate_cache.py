...
Problem recap
-------------
`_fused_marlin_moe` in `experts/marlin_moe.py` allocates two intermediate
...
------------------------
If `experts/marlin_moe.py` gains an upstream shared-pool implementation
(hypothetical PR #40655-class for MoE), the markers in
`UPSTREAM_DRIFT_MARKERS` self-retire our patch.

Rebase note (2026-06-07): `_fused_marlin_moe` already accepts
`intermediate_cache13` / `intermediate_cache2` as optional caller-supplied
kwargs in vllm-new (experts/marlin_moe.py:91-92), with the `is None`
fallback to `torch.empty` that we anchor on. The layer-level resolution
the retirement note anticipated has NOT landed — the per-call lazy
`torch.empty` blocks still exist verbatim, so this patch remains
applicable when `GENESIS_ENABLE_P37=1`.
...
def _make_patcher() -> TextPatcher | None:
    # Rebase 2026-06-07: the marlin MoE kernel lives in
    # `experts/marlin_moe.py::_fused_marlin_moe` (the standalone
    # `fused_marlin_moe.py` module was folded into `experts/` upstream
    # before our OLD baseline). The two `if cache is None: torch.empty(...)`
    # anchor blocks are byte-identical there in vllm-new
    # (experts/marlin_moe.py:105-117). Targeting the old path made this
    # patch self-skip with "not found" on every boot.
...
def apply() -> tuple[str, str]:
    ...
    if patcher is None:
        return "skipped", "experts/marlin_moe.py not found"
