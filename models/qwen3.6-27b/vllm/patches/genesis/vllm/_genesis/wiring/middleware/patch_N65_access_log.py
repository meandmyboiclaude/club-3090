...
def apply() -> tuple[str, str]:
    ...
    # [2026-07-14] SUPERSEDED by the sndr v3 logging-filter PN65 (lane-2,
    # same env flag): importing api_server here re-initializes vLLM logging
    # mid-apply and BLACKS OUT every genesis log line after this patch —
    # which hid a real spec-loop failure behind a silent crash-loop. The v3
    # filter delivers the structured access log with no api_server import
    # and zero request-path overhead. This middleware variant stays retired.
    return (
        "skipped",
        "PN65 v2 middleware superseded by sndr v3 logging-filter (lane-2, "
        "same env flag) — api_server import at apply time re-inits vLLM "
        "logging and silences the remaining apply log",
    )
