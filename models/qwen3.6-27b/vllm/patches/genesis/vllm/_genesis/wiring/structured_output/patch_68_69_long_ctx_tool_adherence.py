...
GENESIS_P6869_MARKER = "Genesis P68/P69 long-context tool-call adherence v7.14_b4c80ec"


# ─── Sub-patch: insert hook call at top of _create_chat_completion ──────────
# RE-ANCHORED for nightly-b4c80ec (2026-06-18): upstream split the public
# `create_chat_completion` into a thin wrapper that delegates to a new private
# `_create_chat_completion(self, request, raw_request)` via
# `_with_kv_transfer_rejection_cleanup`. The docstring now lives on the public
# wrapper; all real request processing (including the `# Streaming response` /
# `tokenizer = self.renderer.tokenizer` first action) moved into the private
# method with NO docstring. This is a pure MOVE — the injection point (method
# entry, before the first action) is unchanged in intent, so we re-anchor on
# the private method's signature + first action. The hook still runs before any
# request processing for both streaming and non-streaming paths.
# Anchor on the `_create_chat_completion` signature + "# Streaming response"
# comment + tokenizer fetch. Insert hook call AFTER signature but BEFORE first
# action.
P6869_OLD = (
...
def apply() -> tuple[str, str]:
    ...
    if patcher.marker in content:
        ...
        if patcher.sub_patches[0].anchor not in content:
            return (
                "skipped",
                "required anchor (_create_chat_completion signature + tokenizer "
