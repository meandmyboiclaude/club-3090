...
===============================================================================
DISPOSITION 2026-07-14 (dev1060cherry): SUPERSEDED-NATIVE — self-skips, KEEP OFF.
Upstream rewrote `structured_output/__init__.py::grammar_bitmask` to the parallel
batched `_fill_bitmasks` path (fill_bitmask_parallel_threshold=128), which fixes
the same MTP×xgrammar timing bug this patch targeted (MTP proposing a token past
the grammar stop → FSM 500). VERIFIED on dev1060cherry: guided-JSON (json_schema,
strict) + tool_choice=required, 4-way concurrent × 20 each under MTP n=3 →
40/40 HTTP 200, 20/20 valid JSON, ZERO 500s. 3 of 5 anchors are drifted against
the rewrite; re-anchoring blind would risk BREAKING the now-correct grammar path
with no repro to gate it. The fix P62 provided is delivered by upstream → this
patch's improvement is already in the build. Re-open only if a tool-call/guided
500 ever reproduces under MTP+structured-output on this pin.
===============================================================================
...
# Re-anchored for dev491 (nightly-1033ffac): upstream split grammar_bitmask
# into a parallel fast-path + serial fallback, added a diffusion-LLM token_iter
# branch, and added a post-loop `grammar.rollback(state_advancements)`. This
# anchor matches the SERIAL fallback (the spec-decode path our MTP config always
# takes, since max_num_spec_tokens > 0 skips the parallel branch).
GRAMMAR_BITMASK_OLD = (
...
GRAMMAR_BITMASK_NEW = (
    "                # [Genesis P62 vllm#36138, v7.70 post-PR41199; re-anchored dev491]\n"
    "                # reasoning-aware bitmask + advance. dev491 added the diffusion\n"
    "                # token_iter branch and a post-loop grammar.rollback; both kept.\n"
    "                req_tokens = scheduled_spec_decode_tokens.get(req_id, ())\n"
