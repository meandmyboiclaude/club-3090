...
KNOWN_GOOD_VLLM_PINS: tuple[str, ...] = (
    ...
    "0.20.2rc1.dev9+g01d4d1ad3",
    # cu129 nightly 2026-05-10 (manually added for aibox)
    "0.20.2rc1.dev189+gfb1ac806c",
    # PROD aibox build 2026-05-26 (commit aa2b56ffb, ~v0.22.0-rc; in service
    # since 2026-05-26 — grandfathered known-good)
    "0.21.1rc1.dev281+gaa2b56ffb",
    # nightly-9c7f7741 (2026-06-07) — vllm-main rebase target (aibox);
    # validated by Phase-3 boot+smoke+bench of genesis-rebase-vllm-main
    "0.22.1rc1.dev245+g9c7f7741d",
    # nightly-1033ffac (2026-06-13) — vllm-main rebase target (aibox). Validated:
    # apply_all byte-identical to dev245 (18 applied / 101 skipped / 0 failed / 5 ⚠️,
    # same drift IDs P103/P15/P28/P44/P46); all 6 club-3090 /fixes/ apply; GDN source
    # unchanged. Patched target files turboquant_attn/gpu_worker/reasoning/GDN = 0 diff.
    "0.22.1rc1.dev491+g1033ffac2",
    # nightly-b53b1c7 (2026-06-18) — Qwopus3.6-27B-v2-GPTQ-Pro-MTP-BF16 adoption.
    # Validated: 36 Genesis applied / 0 failed / 13 partial-warn (all benign — Rust-
    # migrated parser files + GDN/TQ path moves); PN71-74 apply; MTP n=3 + TQ3 KV;
    # health 200, reasoning + streaming <tool_call> + thinking-len correct.
    "0.23.1rc1.dev178+gb53b1c7ff",
    # nightly-3f5a1e17 (2026-06-26) — dev424 promote. CRASH FIX: disabled racy
    # /fixes/tq_buffer_pool (async-overlap device-assert, BUG-028) + PN75 embedding
    # -1 guard (defensive). util 0.91 (KV 152K/2.03x). Validated: 6/6 functional
    # (incl streaming <tool_call> via PN73/PN73T) + killer listwise+thinking_token_budget
    # rerank clean. OWED: full 320-call soak at 0.91 + leak/stability check (see handoff).
    "0.23.1rc1.dev424+g3f5a1e173",
    # nightly-69715823 (2026-07-06) — dev799 bump candidate (2026-07-07 session).
    # Carries structured-output×spec-decode bitmask fix (#44297), EngineCore
    # degenerate-structured-outputs rejection (#45346), TQ dtype fix (#47609).
    # PN8 imports re-anchored (QuantizationConfig moved under TYPE_CHECKING).
    # UNPROVEN until functional 6/6 + bench ±5% + soak pass; boot-guard stays
    # pinned to 3f5a1e17 until promoted.
    "0.23.1rc1.dev799+g69715823d",
    # nightly-9e57de71 (2026-07-13) — dev1060 bump. Brings 481e481b hybrid
    # partial prefix-cache + 51878e5b KV layout refactor. PN8/PN12/P34/P83
    # re-anchored this session (PN8 anchor stays dev799-compatible; the other
    # three target the dev1060 form only). PN80 re-anchored in club-3090/fixes.
    "0.23.1rc1.dev1060+g9e57de719",
...
def resolve_vllm_file(relative_path: str) -> Optional[str]:
    ...
    if os.path.exists(full):
        return full
    # Upstream relocations. Aliased BOTH directions so this one shared genesis
    # tree serves pins on either side of a move. Mirrors the same table in
    # sndr/engines/vllm/detection/guards.py.
    #
    # [2026-07-25] vllm#48500 moved fla ops out of model_executor into
    #   third_party/ (same files, new home).
    # [2026-07-26] the GDN attention module was split per-family:
    #   model_executor/layers/mamba/gdn_linear_attn.py became
    #   model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py (siblings
    #   kimi_/olmo_). The rename was already recorded in dispatcher.py's P60
    #   row but never propagated to the resolver, so P7 and P7b resolved to
    #   None and reported "gdn_linear_attn.py not found" on every boot.
    #   This is a FILE RENAME, not a directory move, so it is listed as an
    #   exact-path pair rather than a prefix.
    for old, new in _PATH_ALIASES:
        alt = None
        if old.endswith("/") and relative_path.startswith(old):
            alt = os.path.join(root, new + relative_path[len(old):])
        elif not old.endswith("/") and relative_path == old:
            alt = os.path.join(root, new)
