...
PATCH_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "P40": {
        ...
        "lifecycle": "legacy",
        "category": "kernel_perf",
        "credit": "Pre-dispatcher legacy patch (vllm#40792 backport candidate). Opt-in GQA-grouped TQ decode stage1 kernel. Welch t-test on 2x A5000 single-stream: not significant (p=0.284 vs baseline 183 TPS) — that test covered the k8v4 path only, short-ctx. v7.73 (2026-07-13): generalized to MSE-key presets (turboquant_3bit_nc / turboquant_k3v4_nc, VQB 3/4) with a tuned launch table; RTX 4090 idle microbench vs scalar launcher: 2.87x at 32K ctx bucket-8 (2596->904 us), 1.93x at B=1; regression below ~500-token ctx (350 tok: +25 us/call). Upstream #40792 self-retire covers k8v4 only — re-evaluate before retiring the MSE path when it merges.",
    },
    "P44": {
...
def should_apply(patch_id: str) -> tuple[bool, str]:
    ...
    # [sndr-v12-import 2026-07-13] migration lever: GENESIS_SNDR_OWNS_<bare>=1
    # hands this patch to lane-2 (the vendored sndr v12 dispatcher) — lane 1
    # must then skip it or both lanes would text-patch the same site.
    # See patches/sndr_lane.py for the matching half of the lever.
    if env_flag:
        _bare = env_flag
        for _pfx in ("SNDR_ENABLE_", "GENESIS_ENABLE_"):
            if _bare.startswith(_pfx):
                _bare = _bare[len(_pfx):]
                break
        if os.environ.get(f"GENESIS_SNDR_OWNS_{_bare}", "").strip().lower() in (
            "1", "true", "yes", "on",
        ):
            return False, "delegated to sndr lane-2 (GENESIS_SNDR_OWNS override)"
