...
class Gemma4ToolParser(ToolParser):
    ...
    def _extract_streaming(
        ...
        # Note: do NOT require start_count > end_count here — when the entire
        # tool call arrives in one streaming chunk (common with stream-interval
        # batching), start and end counts are equal in the same delta.  We
        # still need to initialise the tool state so _handle_tool_call_end
        # (Case 3 below) can emit the complete call correctly.
        if start_count > prev_start_count:
            new_calls = start_count - prev_start_count
            ...
            # If the delta contains only start token(s) and nothing more, wait.
            # Otherwise fall through — end tokens may also be present.
            if len(delta_text) <= len(self.tool_call_start_token) * new_calls:
                return None
    ...
    def _handle_tool_call_end(self, current_text: str) -> DeltaMessage | None:
        ...
        Single-delta case: when start and end tokens arrive in the same chunk
        (e.g. with stream-interval batching), Case 2 sets up the state but
        current_tool_name_sent is still False.  In that case emit name + args
        together in one DeltaToolCall so the caller sees a complete tool call.
        """
        ...
        if self.current_tool_id < len(all_matches):
            ...
            # Single-delta case: one or more tool calls arrived without being
            # individually streamed (start+end in same chunk, or N calls all
            # arriving in one batched delta).  Emit every call whose name has
            # not been sent yet (prev_tool_call_arr entry still empty).
            if not self.current_tool_name_sent:
                self.current_tool_name_sent = True
                unstreamed: list[DeltaToolCall] = []
                for idx, (fname, astr) in enumerate(all_matches):
                    if idx > self.current_tool_id:
                        break
                    if self.prev_tool_call_arr[idx].get("name"):
                        continue  # already streamed individually
                    fargs = _parse_gemma4_args(astr)
                    fargs_json = json.dumps(fargs, ensure_ascii=False)
                    self.prev_tool_call_arr[idx] = {"name": fname, "arguments": fargs}
                    self.streamed_args_for_tool[idx] = fargs_json
                    unstreamed.append(
                        DeltaToolCall(
                            index=idx,
                            type="function",
                            ...
                            function=DeltaFunctionCall(
                                name=fname,
                                arguments=fargs_json,
                            ).model_dump(exclude_none=True),
                        )
                    )
                return DeltaMessage(tool_calls=unstreamed) if unstreamed else None
