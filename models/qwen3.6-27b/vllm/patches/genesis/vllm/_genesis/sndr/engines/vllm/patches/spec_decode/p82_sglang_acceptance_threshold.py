...
def _make_patcher(threshold: float, min_draft_pos: int = 0) -> TextPatcher | None:
    ...
    return TextPatcher(
        ...
        upstream_drift_markers=[
            ...
            "SpecVerifyMethod",
            # PR #41258 (masterFoad, OPEN as of 2026-05-09):
            # "Lazy recovery evaluation for spec rejection sampling"
            # would replace the eager `sample_recovered_tokens_kernel`
            # with lazy in-kernel evaluation. Match ONLY the new lazy
            # markers — DO NOT match `sample_recovered_tokens_kernel`
            # itself, which is the EXISTING base function name in
            # every vllm pin (false-positive drift detection landed
            # P82 in the SKIP bucket on dev93 starting 2026-05-08,
            # masking ~8-12% TPS regression). When PR #41258 actually
            # merges, vllm will introduce one of the markers below
