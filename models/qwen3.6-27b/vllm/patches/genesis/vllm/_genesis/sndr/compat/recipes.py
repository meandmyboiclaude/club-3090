...
"""Genesis recipes — `python3 -m sndr.compat.recipes`.
...
Workflow:
  ...
  python3 -m sndr.compat.recipes save prod-v794 \\
      --from-container vllm-server-mtp-test \\
      --description "27B Lorbus INT4 + TQ k8v4 + 5 PN-family patches"

  # List local recipes
  python3 -m sndr.compat.recipes list

  # Display one recipe
  python3 -m sndr.compat.recipes show prod-v794

  # Generate launch script from a recipe
  python3 -m sndr.compat.recipes load prod-v794 --out start.sh

  # Delete
  python3 -m sndr.compat.recipes delete old-recipe
...
def to_launch_script(recipe: dict[str, Any]) -> str:
    ...
    L.append(f"# Generated by: python3 -m sndr.compat.recipes load {name}")
    L.append("# ════════════════════════════════════════════════════════════════════")
    ...
    inner = ["set -e"]
    inner.append("python3 -m sndr.apply")
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="python3 -m sndr.compat.recipes",
    ...
    if args.cmd == "adopt":
        ...
        print(f"  Inspect: python3 -m sndr.compat.recipes "
              f"show {args.name}")
        return 0
