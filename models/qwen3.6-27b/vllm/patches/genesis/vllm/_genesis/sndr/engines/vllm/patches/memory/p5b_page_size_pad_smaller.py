...
Lifecycle: COORDINATOR (per dispatcher.PATCH_REGISTRY['P5b']['lifecycle']).
This module is intentionally a feature-flag handle: it does NOT modify any
runtime class itself. The actual text-patch lives in P5 wiring and reads
`is_p5b_enabled()` at apply-time to choose between v1 (LCM-pad-up) and
v2 (pad-smaller-to-max) bodies. P5b ships separately so:
  - Operators can grep `GENESIS_ENABLE_P5B` and find the feature.
  - Audit / preflight can flag P5b composability with other KV patches.
  - The design rationale below stays attached to the toggle, not buried
    inside P5's omnibus docstring.
