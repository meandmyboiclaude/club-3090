...
def _probe_quant_format(cfg: Any, hf_config: Any) -> str:
    ...
    if q:
        ...
        # AutoRound canonical aliases in vllm: "auto_round" (underscore),
        # "auto-round" (hyphen), "autoround" (legacy/no-sep). Bug fix
        # 2026-06-05: previously only "autoround" was searched, which
        # silently missed the most common vllm canonical "auto_round"
        # form due to substring mismatch (underscore present in target).
        # vllm/compat/preflight_checks.py:76 enumerates all 3 aliases.
        for marker, label in (
            ("auto_round", "autoround"),       # vllm canonical (underscore)
            ("auto-round", "autoround"),       # vllm alias (hyphen)
            ("autoround", "autoround"),        # legacy alias (no separator)
    ...
    if qcfg:
        ...
        # AutoRound canonical name forms in HF quantization_config (3 variants):
        # "auto_round" (underscore, primary), "auto-round" (hyphen — what Qwen3.6
        # AutoRound INT4 checkpoints emit), "autoround" (legacy no-separator).
        # Bug fix 2026-06-05: previously only first two were checked, missing
        # the hyphen variant used by Lorbus 27B INT4 production model.
        # Also: default fallback now derives from `bits` field if available
        # (was always "autoround_int8" — wrong for INT4 models without bits).
        if "autoround" in qm or "auto_round" in qm or "auto-round" in qm:
            try:
            ...
            return "autoround_int4"  # default INT4 — vast majority of AutoRound
