...
# PN59 `o_output` pool cap (Genesis 2026-05-07).
#
# WHY: `acquire_o_output` bins T to `next_pow2(T)` to amortize grow events.
# On long-ctx single-card configs (e.g. 1×A5000 24GB + max_model_len ≥ 64K),
# this can quietly allocate up to ~768 MiB persistent (the largest unbounded
# grower in the GDN stack — confirmed by 2026-05-07 memory audit).
#
# MITIGATION: operator-set ceiling (in TOKENS, not bytes — caller passes T).
# When T_binned would exceed the cap, `acquire_o_output` raises; caller's
# existing try/except falls back to `torch.empty_like(v)` for that one
# request — pool stays bounded for typical (short) requests.
#
# DEFAULT 0 = unset = unlimited (current pre-2026-05-07 behavior, zero-risk).
# Recommended for 1×24GB hybrid configs: 65536 (caps at 64K-binned).
_ENV_O_MAX_T = "GENESIS_PN59_O_MAX_T"

# Read once at module load (env stable post-spawn). Reset only by re-import.
_O_MAX_T_CACHE: int | None = None


def _get_o_max_t() -> int:
    """Return GENESIS_PN59_O_MAX_T cap in tokens; 0/unset/invalid = unlimited."""
    global _O_MAX_T_CACHE
    if _O_MAX_T_CACHE is None:
        try:
            _O_MAX_T_CACHE = max(0, int(os.environ.get(_ENV_O_MAX_T, "0").strip()))
        except (ValueError, TypeError):
            _O_MAX_T_CACHE = 0
    return _O_MAX_T_CACHE


def _reset_o_max_t_cache() -> None:
    """Test helper — clear cache so a new env value is re-read."""
    global _O_MAX_T_CACHE
    _O_MAX_T_CACHE = None
...
class GdnScratchPool:
    ...
    def acquire_o_output(
        ...
        # Genesis 2026-05-07 — bounded-growth cap. See _ENV_O_MAX_T docstring.
        # Default 0 (unlimited) preserves pre-2026-05-07 behavior.
        cap = _get_o_max_t()
        if cap > 0 and T_binned > cap:
            raise RuntimeError(
                f"[PN59] o_output T_binned={T_binned} exceeds "
                f"GENESIS_PN59_O_MAX_T={cap} — bypassing pool to keep "
                f"persistent VRAM bounded; caller falls back to vanilla "
                f"torch.empty_like for this request"
            )
        ...
        # Audit A-15 fix Phase 2 (2026-05-06 evening): pool_budget guard
        # for GDN scratch o_output pool. Operator can cap via env
        # GENESIS_POOL_MAX_MIB_GDN_SCRATCH_O or global GENESIS_POOL_MAX_MIB.
        # Re-wired from dequant_buffer (which doesn't fire on current 27B
        # stack) to gdn_scratch_pool.acquire_o_output (proven hot path —
        # this is the pool that allocates the 24 MiB chunk_o buffer per
        ...
        try:
            ...
            if type(_e).__name__ == "PoolBudgetExceeded":
                raise
            log.warning(
                "[pool_budget] check failed in gdn_scratch.o_output: %s — "
                "proceeding without guard", _e,
            )
        ...
        try:
            ...
            log.debug(
                "[pool_budget] record failed in gdn_scratch.o_output: %s", _e,
            )
