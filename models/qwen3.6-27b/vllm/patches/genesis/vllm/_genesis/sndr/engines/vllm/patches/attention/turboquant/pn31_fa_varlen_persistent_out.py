...
PN31_REPLACEMENT_NO_VER = (
    ...
    "        # [2026-07-14 v3 capture-safety] Attention backends run EAGER (custom-op\n"
    "        # boundary, outside dynamo), so the pool executes on every varlen prefill.\n"
    "        # Two live-hit failure modes closed here (boots 8-15 forensics):\n"
    "        #   1. allocation while a CUDA stream is capturing -> illegal; guard with\n"
    "        #      is_current_stream_capturing() (is_compiling alone was insufficient).\n"
    "        #   2. per-exact-shape keys: varlen total_q diversity made the pool grow\n"
    "        #      unbounded at 0.91 util -> allocator failure surfacing as stable-ABI\n"
    "        #      aten::empty RuntimeError inside the wheel custom op. Fix: bucket\n"
    "        #      total_q to 1024-multiples (few keys, view-sliced), try/except with\n"
    "        #      out=None fallback (stock behavior) so pool pressure NEVER fails a\n"
    "        #      request.\n"
    "        import torch as _genesis_pn31_torch\n"
    "        _genesis_pn31_out = None\n"
    "        if not (\n"
    "            _genesis_pn31_torch.compiler.is_compiling()\n"
    "            or _genesis_pn31_torch.cuda.is_current_stream_capturing()\n"
    "        ):\n"
    "            try:\n"
    "                _genesis_pn31_tq = q.shape[0]\n"
    "                _genesis_pn31_bucket = -(-_genesis_pn31_tq // 1024) * 1024\n"
    "                _genesis_pn31_buf_key = (\n"
    "                    _genesis_pn31_bucket, q.shape[1], q.shape[2],\n"
    "                    q.dtype, q.device,\n"
    "                )\n"
    "                if not hasattr(self, '_genesis_pn31_out_bufs'):\n"
    "                    self._genesis_pn31_out_bufs = {}\n"
    "                _genesis_pn31_buf = self._genesis_pn31_out_bufs.get(\n"
    "                    _genesis_pn31_buf_key\n"
    "                )\n"
    "                if _genesis_pn31_buf is None:\n"
    "                    if len(self._genesis_pn31_out_bufs) >= 8:\n"
    "                        # bounded pool: drop everything on overflow (rare;\n"
    "                        # buckets make >8 keys pathological) rather than grow.\n"
    "                        self._genesis_pn31_out_bufs.clear()\n"
    "                    _genesis_pn31_buf = _genesis_pn31_torch.empty(\n"
    "                        (_genesis_pn31_bucket, q.shape[1], q.shape[2]),\n"
    ...
    "                        _genesis_pn31_buf\n"
    "                    )\n"
    "                _genesis_pn31_out = _genesis_pn31_buf.narrow(\n"
    "                    0, 0, _genesis_pn31_tq\n"
    "                )\n"
    "            except Exception:\n"
    "                # pool pressure or capture edge: stock path, never fail.\n"
    "                _genesis_pn31_out = None\n"
    "        if self.fa_version is None:\n"
    "            return flash_attn_varlen_func(\n"
    "                q=q,\n"
