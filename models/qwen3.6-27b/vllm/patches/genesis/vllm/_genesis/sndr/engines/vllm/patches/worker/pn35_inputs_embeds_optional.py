...
- Registry env_flag: `GENESIS_ENABLE_PN35_INPUTS_EMBEDS_OPTIONAL`
  (dispatcher-gated; default OFF until live A/B confirms text-only-
  model VRAM win across full PROD config matrix).
...
# at BUILD TIME (not runtime) to enroll PN35's two sub-patchers into the
# anchor-offset manifest. It constructs the same TextPatcher objects
# pointed at PRISTINE FIXTURE paths under tests/legacy/pristine_fixtures/
# so it works without a live vllm install.
#
# Runtime apply() above is unaffected — it still uses resolve_vllm_file()
# to find the live install. The two paths are orthogonal.


def _make_patcher_for_fixture(
    name: str, fixture_path, anchor, replacement, *, patch_id: str,
) -> TextPatcher:
    """Build a TextPatcher targeting a pristine fixture file (build mode)."""
    return TextPatcher(
        patch_name=name,
        target_file=str(fixture_path),
        marker=GENESIS_PN35_MARKER,
        sub_patches=[
            TextPatch(
                name="pN35_inputs_embeds_optional",
                anchor=anchor,
...
def register_for_manifest(*, pristine_root) -> None:
    """Register PN35's two sub-patchers into the Site Map registry,
    using pristine fixtures from `pristine_root`.

    Called by `scripts/build_anchor_manifest.py`. Idempotent: re-calling
    with the same patchers is a no-op.
    ...
    register_text_patcher(
        ...
        _make_patcher_for_fixture(
            "PN35 Sub-1 gpu_model_runner.py (build mode)",
            pristine_root / "gpu_model_runner.py",
            PN35_PART1_ANCHOR,
            PN35_PART1_REPLACEMENT,
            patch_id="PN35.Sub-1",
        ),
    ...
    register_text_patcher(
        "PN35.Sub-2",
        _make_patcher_for_fixture(
            "PN35 Sub-2 llm_base_proposer.py (build mode)",
            pristine_root / "llm_base_proposer.py",
            PN35_PART2_ANCHOR,
            PN35_PART2_REPLACEMENT,
            patch_id="PN35.Sub-2",
        ),
