...
# `GENESIS_LEGACY_P38` is synthetic — flag exists for registry/audit
...
# Audit 2026-05-11 (contract test): torch imports moved to lazy
# function-body to preserve torch-less pytest collection. CLI / doctor /
# pre-commit run without torch available; module-level `import torch`
# would crash imports in those contexts. Inside `apply()` torch is
# present (runtime container always has it).
