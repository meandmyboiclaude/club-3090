...
class TurboQuantBufferManager:
    ...
    def get_or_create_kv_buffers(
        ...
        if key not in cls._K_BUFFERS:
            ...
            # Audit A-15 fix 2026-05-06 (POC): pool_budget guard before allocation.
            # Operator can cap per-pool VRAM via GENESIS_POOL_MAX_MIB_DEQUANT_KV
            # or global GENESIS_POOL_MAX_MIB. Total cap via
            ...
            try:
                ...
                # boot rather than silently overrun budget). Other exceptions
                # log only — never break vllm on diagnostic helper failure.
                if type(_e).__name__ == "PoolBudgetExceeded":
                    raise
                log.warning(
                    "[pool_budget] check failed: %s — proceeding without guard",
                    _e,
                )
            ...
            try:
                ...
                log.debug(
                    "[pool_budget] record failed (best-effort): %s", _e
                )
