...
if TYPE_CHECKING:
    # P0-1 (audit 2026-05-08): torch is imported only inside the methods
    # that actually allocate tensors. Module-level `import torch` broke
    # torch-less environments (CLI, schema validator, doctor, pre-commit
    # hooks). With `from __future__ import annotations` all type
    # annotations are strings at module load, so the TYPE_CHECKING import
    # is sufficient for static analysis without affecting runtime.
...
class GenesisPreallocBuffer:
    ...
    def get_or_create(
        ...
        """
        # P0-1: function-local torch import. Caller is responsible for
        # being in a torch-equipped runtime (this code only runs inside
        # vllm worker processes); module load itself does not need torch.
        import torch
