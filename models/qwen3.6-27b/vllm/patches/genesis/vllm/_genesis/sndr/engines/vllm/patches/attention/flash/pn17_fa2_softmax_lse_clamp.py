...
GENESIS_PN17_MARKER = "Genesis PN17 FA2 softmax_lse runtime clamp v1_v11.3.0_hotpath"
...
PN17_NEW = (
    ...
    "            seqused_k = attn_metadata.seq_lens\n"
    "            max_seqlen_q = attn_metadata.max_query_len\n"
    "            # [Genesis PN17 v2 — FA2 softmax_lse runtime clamp, hot-path optimized]\n"
    "            # FA2 varlen allocates softmax_lse[num_seqs, heads, max_seqlen_k]\n"
    "            # — sized by THIS arg, not by actual seqused_k. Eager runtime:\n"
    "            # clamp to actual chunk max from seqused_k. Capture mode: keep\n"
    "            # max_model_len for shape stability.\n"
    "            # v2: torch.cuda.is_available() is boot-stable; cache in upstream-\n"
    "            # file globals so per-attention-call CUDA-driver check (~200ns)\n"
    "            # is eliminated. is_current_stream_capturing() must stay dynamic.\n"
    "            import torch as _genesis_pn17_torch\n"
    "            _g_pn17_avail = globals().get('_GENESIS_PN17_cuda_avail')\n"
    "            if _g_pn17_avail is None:\n"
    "                try:\n"
    "                    _g_pn17_avail = bool(_genesis_pn17_torch.cuda.is_available())\n"
    "                except Exception:\n"
    "                    _g_pn17_avail = False\n"
    "                globals()['_GENESIS_PN17_cuda_avail'] = _g_pn17_avail\n"
    "            if _g_pn17_avail:\n"
    "                try:\n"
    "                    _genesis_pn17_capturing = (\n"
    "                        _genesis_pn17_torch.cuda.is_current_stream_capturing()\n"
    "                    )\n"
    "                except Exception:\n"
