...
# Kernel module imports torch at top level; importing it eagerly breaks
# torch-less environments (CI lint, doc-build, Mac dev). The env gate is
# replicated locally here; the kernel module is imported lazily inside
# apply() after the gate clears.
_ENV_ENABLE_P7B = "GENESIS_ENABLE_P7B"


def is_p7b_enabled() -> bool:
    """Env-only gate. Mirrors `kernels.gdn_dual_stream_customop.is_p7b_enabled`
    without pulling torch — both must accept the same env values."""
...
def _make_patcher() -> TextPatcher | None:
    # K.1.R.R.7 (2026-05-29): #41126 split mamba/gdn_linear_attn.py.
    # See K.1.R.R.1/2A for the same fallback pattern on P28/P46/P60/P60b/PN11
    # and K.1.R.R.4 for P7/PN32/PN50/PN54/PN79/PN200/PN204.
    target = (
...
def is_applied() -> bool:
    ...
    # K.1.R.R.7: fallback to new path post-#41126 mamba/gdn/ reorg.
    target = (
        resolve_vllm_file("model_executor/layers/mamba/gdn_linear_attn.py")
        or resolve_vllm_file(
            "model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py"
