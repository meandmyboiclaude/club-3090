...
- **Default**: ON when `num_speculative_tokens > 1` (real correctness
  fix, not experimental). Registry env_flag is
  `GENESIS_ENABLE_PN33_SPEC_DECODE_WARMUP_K` (dispatcher-gated). The
  in-source kill-switch `GENESIS_DISABLE_PN33_SPEC_DECODE_WARMUP_K=1`
  disables if warmup itself OOMs on a tight rig (uncommon escape hatch).
...
# at BUILD TIME (not runtime) to enroll PN33's sub-patcher into the
...
def _make_patcher_for_fixture(
    ...
    return TextPatcher(
        ...
        upstream_drift_markers=[
            "[Genesis PN33",
            # vllm#37521-specific marker — see anchor docstring above
            "spec_decode_tokens = [i for i in range(self.num_spec_tokens)]",
        ],
        patch_id=patch_id,
    )
...
def register_for_manifest(*, pristine_root) -> None:
    """Register PN33's sub-patcher into the Site Map registry, using
    pristine fixture from `pristine_root`.

    Called by `scripts/build_anchor_manifest.py`. Idempotent: re-calling
    with the same patcher is a no-op.
    ...
    register_text_patcher(
        ...
        _make_patcher_for_fixture(
            "PN33 Sub-1 gpu_model_runner.py (build mode)",
