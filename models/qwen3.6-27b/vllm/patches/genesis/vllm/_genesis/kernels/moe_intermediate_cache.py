...
Problem
-------
`experts/marlin_moe.py::_fused_marlin_moe` in upstream vLLM allocates two
intermediate tensors **lazily, per-call** (verified unchanged in vllm-new
2026-06-07, experts/marlin_moe.py:105-117):
...
dynamo-safety
-------------
This is called from within `experts/marlin_moe.py::_fused_marlin_moe`
which is traced by `aot_compile_fullgraph`. We therefore:
...
per-call (see PR #40655 pattern for the TQ decode case). As of vllm-new
2026-06-07 that has NOT landed for MoE intermediates: `_fused_marlin_moe`
exposes `intermediate_cache13/2` as optional kwargs but still falls back
to per-call `torch.empty` when they are None — exactly the block this
pool replaces. When a true layer-level pool lands, the
`upstream_drift_markers` in
`vllm/_genesis/wiring/legacy/patch_37_moe_intermediate_cache.py` will
catch the signature and skip.
