...
if _TRITON_OK:
    ...
    def _pn50_fused_qkvzba_split_reshape_cat_contiguous_kernel(
        ...
        # BLOCK_V = next_power_of_2(V_PER_GROUP * HEAD_V) — tl.arange requires
        # a power-of-2 range; V_PER_GROUP*HEAD_V isn't one on e.g. Qwopus
        # (48v/16qk -> V_PER_GROUP=3, 3*128=384). Pad + mask the tail.
        idx_v = tl.arange(0, BLOCK_V)
        mask_v = idx_v < V_PER_GROUP * HEAD_V
        blk_v_ptr = (
            mixed_qkvz + i_bs * TOTAL_QKVZ + TOTAL_Q + TOTAL_K
            + i_qk * V_PER_GROUP * HEAD_V + idx_v
        )
        blk_z_ptr = (
        ...
        tl.store(blk_v_st_ptr, tl.load(blk_v_ptr, mask=mask_v, other=0), mask=mask_v)
        tl.store(blk_z_st_ptr, tl.load(blk_z_ptr, mask=mask_v, other=0), mask=mask_v)
...
def fused_qkvzba_split_reshape_cat_contiguous(
    ...
    """
    # [2026-07-14 compile-safety] Under torch.compile, use the stock unfused
    # chain: dynamo traced the wrapper's torch.empty output allocations into
    # the compiled graph -> baked aten::empty crashed cudagraph replay
    # (stable-ABI RuntimeError, boots 8/9/12). is_compiling() constant-folds,
    # so compiled regions get the fully-traceable stock ops (inductor fuses
    # them anyway); eager callers keep the fused Triton kernel.
    ...
    try:
        ...
        # V_PER_GROUP*HEAD_V padded to the next power of 2 (tl.arange limit);
        # the kernel masks the tail. Required for non-pow2 products, e.g.
        # Qwopus 48v/16qk*128 -> 384 -> BLOCK_V=512.
        block_v = triton.next_power_of_2((num_heads_v // num_heads_qk) * head_v)
        _pn50_fused_qkvzba_split_reshape_cat_contiguous_kernel[grid](
            mixed_qkv, z, b, a,
            mixed_qkvz, mixed_ba,
            num_heads_qk, num_heads_v, head_qk, head_v, block_v,
