...
Wiring reality (rebase note 2026-06-07): only the num_warps/num_stages
helpers are actually injected into vLLM, via the P24 text-patch on
`fused_moe.get_default_config` (Triton fused_moe path). `get_optimal_block_size_m`
is currently consumed only by `apply_patch_17_18_marlin_tuning` for an
observability log line — there is no live monkey-patch that injects `bsm`
into the Marlin `_fused_marlin_moe` call (`block_size_m` there is supplied
by the FusedMoE layer / MoEPrepareAndFinalize, not by this module). The
bsm table is therefore advisory until a future text-patch wires it.

#43731 (merged 2026-05-27, "Enable TritonW4A16LinearKernel as CUDA
fallback for non-Marlin-aligned W4A16 shapes") touches ONLY
`kernels/linear/` (dense Linear). It does NOT alter `get_default_config`,
`experts/marlin_moe.py`, or any MoE tile selection, so it does NOT change
P17/P18 or P24 tuning assumptions. Qwen3.6-27B MoE W4A16 still routes
through the int4_w4a16 branch of `get_default_config` (which sets no
num_warps/num_stages — see P24 wiring) and the Marlin MoE CUDA op.
