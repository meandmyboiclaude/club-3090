...
**Problem (measured, torch-profiler 2026-07-13 on the live :8020 4090 TQ3
deployment — see club-3090 `diagnostics/tq-lane/PROFILE-ANALYSIS.md`):**
the scalar `_tq_decode_stage1` is 60% of the decode step at 32K ctx
(2,029 us/call median, ~13% of achievable bandwidth; `num_warps=1,
num_stages=1, BLOCK_KV=4`, one CTA per q-head → 6× redundant KV reads at
GQA group 6). `GENESIS_ENABLE_P40=1` was set but the grouped kernel
self-skipped on 100% of calls: the upstream #40792 port implements ONLY
FP8-key scoring + 4-bit value unpack, and `should_use_grouped_kernel`
required `key_fp8 and value_quant_bits == 4` — the live preset is
`turboquant_3bit_nc` (MSE 3-bit keys, `key_fp8=False`, VQB=3).

**Root cause of the VQB==4 gate: real, not conservative** — the kernel
had `tl.static_assert(VQB == 4)`, no 3-bit unpack, and no MSE-key
(centroid + norm) dequant at all.

**Change (generalize, one kernel):**
- `kernels/tq_grouped_decode.py`: `_tq_grouped_decode_stage1` gains the
  MSE-key path (3/4-bit index unpack → centroid gather → optional
  norm-correction → fp16 vec-norm scale; scores via fp16 `tl.dot`, fp32
  accumulate) and the VQB==3 value unpack (LE 16-bit window, 8 vals /
  3 bytes) — both ported op-for-op from the scalar kernel.
  `static_assert` now `VQB in {3,4}`, `MSE_BITS in {3,4}` when MSE.
- Dispatcher: `key_fp8→VQB==4` (unchanged upstream scope);
  `MSE → VQB∈{3,4} ∧ mse_bits∈{3,4}` (mse_bits=None rejected).
- `resolve_launch_config()`: launch table keyed on (head_dim, group)
  + env overrides `GENESIS_P40_BLOCK_H/BLOCK_KV/NUM_WARPS/NUM_STAGES`
  (validated; invalid → default, never raise). k8v4 keeps upstream
  constants. `(256, 6)` = `(BH=16, BKV=16, W=2, S=1)` from a 54-config
  idle sweep on RTX 4090 — `num_stages=1` is decisive at long ctx
  (S≥2 costs 2-3×), BKV=16 beats 32/64.
- `wiring/legacy/patch_40_tq_grouped_decode.py`: MSE q_rot prep
  (`q.float() @ PiT`, identical to the scalar launcher), centroids +
  MSE layout constants into the kernel, and a fail-loud launcher
  signature/symbol drift anchor (`_check_launcher_drift`) — any
  upstream drift → loud "skipped", scalar stays in charge.

**Validation (side container, pinned dev1060 image, live server idle):**
- Unit: `tests/test_tq_grouped_decode.py` 24/24 (CPU, container).
- Parity: club-3090 `diagnostics/tq-lane/tq_parity_harness.py` grouped
  vs baseline — identical 18/22 PASS pattern (the 4 fails are
  pre-existing stock issues: 2× PN86/#46461 mixed-batch, 2×
  store-bytes ~30/450K bytes — store path, unrelated to decode).
  Decode-path checks: determinism bitwise PASS, continuation q≤128
  max_abs 4.8e-4 / cos_min 1.00000. Direct grouped-vs-scalar A/B:
  allclose(atol 5e-3) + cos_min ≥ 0.999999 at ctx {350, 32K} ×
  B {1, 8}.
- Perf (idle, full launcher call incl. stage2 + q_rot prep):
  32K ctx B=8 (MTP K+1 bucket): 2,596 → 904 us = **2.87×**;
  32K B=1: 492 → 255 us = 1.93×; 350-token ctx: **regression**
  0.69-0.71× (+25 us/call; break-even ≈ 500 tokens ctx).
  Projected step impact @32K: −32 ms of the 64.5 ms step ⇒ ~2×
  decode throughput (exceeds the profile's predicted 1.8×).

Still opt-in (`GENESIS_ENABLE_P40=1`). NOTE: upstream #40792
self-retirement covers k8v4 only — if it merges, the MSE grouped path
retires with it; re-evaluate the gate then.
