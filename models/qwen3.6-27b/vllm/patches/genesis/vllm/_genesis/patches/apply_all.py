...
def run(verbose: bool = True, apply: bool = False) -> PatchStats:
    ...
    # ─────────────────────────────────────────────────────────────────
    # [v7.72.2 fix 2026-05-05, ported to sndr-v12-import 2026-07-14]
    # Structured boot summary emit point.
    #
    # MUST live in run() (not main()) — vllm's plugin loader (genesis_v7
    # entry point → register() → run()) calls run(), never main(). Putting
    # the summary only in main() meant it appeared on `python3 -m
    # vllm._genesis.patches.apply_all` CLI runs but NEVER on real per-process
    # plugin boot (engine-core + every worker rank). Emitting here surfaces
    # it on all entry paths.
    #
    # Falls back to v7.13 apply matrix on any error so boot keeps working.
    # Errors logged at WARNING so operators see them (not the old silent
    # debug log that hid the regression).
    # ─────────────────────────────────────────────────────────────────
    try:
...
def main() -> int:
    ...
    # entry point — which only invokes run() — also gets the summary on
    # every process/worker, not just CLI `python -m ...apply_all` runs.)

    exit_code = 1 if stats.failed_count > 0 else 0

    # [sndr-v12-import 2026-07-13] Lane 2: vendored sndr v12 dispatcher for
    # NET-NEW patches (shared ids suppressed — lane 1 above owns them).
    # Kill-switch: GENESIS_SNDR_LANE=0. See patches/sndr_lane.py docstring
    # + IMPORT-NOTES.md for the two-lane decision record.
    try:
        from vllm._genesis.patches.sndr_lane import run_lane2
        lane2_stats = run_lane2(dry=dry)
        if lane2_stats is not None and lane2_stats.failed_count > 0:
            exit_code = max(exit_code, 1)
    except Exception as e:
        log.exception("[Genesis lane-2/sndr] setup error: %s", e)
        exit_code = max(exit_code, 1)
