...
def test_anchor_targets_finish_reason_block():
    M = _wiring()
    # dev491 re-anchor: vllm#44267 dropped the local `auto_tools_called` flag
    # from chat_completion_stream_generator; the finish-reason head now keys on
    # tools_streamed[i] (+ harmony) instead.
...
def test_replacement_adds_p107_guard():
    ...
    # All AND conditions of the guard must be present. dev491 re-anchor:
    # `and not auto_tools_called` was replaced by `and not tools_streamed[i]`
    # (the unified "no tool emitted" signal) + `and delta_message is not None`.
    ...
    # dev491: reasoning_parser local doesn't exist in the stream generator;
    # guard uses the in-scope self.reasoning_parser_cls instead.
    assert "and self.reasoning_parser_cls is not None" in M.ANCHOR_NEW
    assert "and delta_message is not None" in M.ANCHOR_NEW
