...
def test_pn32_v2_anchor_targets_forward_core_prefill_branch():
    ...
    # (dev491: upstream renumbered "# 2.2" -> "# 2.3")
    ...
    # (dev491: non_spec_state_indices_tensor -> prefill_state_indices)
...
def test_pn32_v2_replacement_bypasses_for_multi_seq():
    ...
    # (dev491: rebased onto attn_metadata.prefill_query_start_loc via the
    #  _genesis_pn32_prefill_qsl local)
    assert "_genesis_pn32_prefill_qsl.shape[0] == 2" in PN32_REPLACEMENT
...
def test_pn32_v2_drift_marker_specific():
    ...
    with tempfile.TemporaryDirectory() as td:
        # dev491: gdn_linear_attn.py moved to gdn/qwen_gdn_linear_attn.py
