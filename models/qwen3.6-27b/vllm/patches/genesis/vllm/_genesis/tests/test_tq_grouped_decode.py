...
- `should_use_grouped_kernel()` dispatcher correctness:
  * opt-in required
  * key_fp8 routes only VQB==4 (upstream scope)
  * MSE keys (v7.73): VQB in {3,4} AND mse_bits in {3,4} route;
    mse_bits=None (unknown) rejected
  * kv_group_size > 1 required (MHA falls through)
- Constants match upstream PR #40792 (BLOCK_H=16, BLOCK_KV=16, etc.)
- `resolve_launch_config()` table defaults + env overrides + invalid
  env values ignored
...
class TestP40DispatcherDecision:
    ...
    def test_mse_key_tq3_routes_to_grouped(self, enabled):
        # v7.73: turboquant_3bit_nc (MSE 3-bit keys, 3-bit values)
        assert enabled.should_use_grouped_kernel(
            kv_group_size=6, key_fp8=False, value_quant_bits=3,
            mse_bits=3,
        ) is True

    def test_mse_key_k3v4_routes_to_grouped(self, enabled):
        # v7.73: turboquant_k3v4_nc (MSE 3-bit keys, 4-bit values)
        assert enabled.should_use_grouped_kernel(
            kv_group_size=6, key_fp8=False, value_quant_bits=4,
            mse_bits=3,
        ) is True

    def test_mse_key_unknown_mse_bits_does_not_route(self, enabled):
        # mse_bits omitted/None = caller didn't say → conservative scalar
        assert enabled.should_use_grouped_kernel(
    ...
    def test_mse_key_unsupported_bits_do_not_route(self, enabled):
        assert enabled.should_use_grouped_kernel(
            kv_group_size=6, key_fp8=False, value_quant_bits=8,
            mse_bits=3,
        ) is False
        assert enabled.should_use_grouped_kernel(
            kv_group_size=6, key_fp8=False, value_quant_bits=3,
    ...
    def test_fp8_non_4bit_values_does_not_route(self, enabled):
        # FP8-key path keeps the original upstream VQB==4-only scope
        assert enabled.should_use_grouped_kernel(
...
class TestP40LaunchConfigResolver:
    """v7.73: resolve_launch_config table + env override behavior."""

    def test_k8v4_keeps_upstream_constants(self, monkeypatch):
        for var in ("GENESIS_P40_BLOCK_H", "GENESIS_P40_BLOCK_KV",
                    "GENESIS_P40_NUM_WARPS", "GENESIS_P40_NUM_STAGES"):
            monkeypatch.delenv(var, raising=False)
        from vllm._genesis.kernels import tq_grouped_decode as k
        assert k.resolve_launch_config(
            head_dim=256, kv_group_size=8, key_fp8=True,
        ) == (k.BLOCK_H, k.BLOCK_KV, k.NUM_WARPS, k.NUM_STAGES)

    def test_mse_qwen36_shape_uses_tuned_entry(self, monkeypatch):
        for var in ("GENESIS_P40_BLOCK_H", "GENESIS_P40_BLOCK_KV",
                    "GENESIS_P40_NUM_WARPS", "GENESIS_P40_NUM_STAGES"):
            monkeypatch.delenv(var, raising=False)
        from vllm._genesis.kernels import tq_grouped_decode as k
        assert k.resolve_launch_config(
            head_dim=256, kv_group_size=6, key_fp8=False,
        ) == k._MSE_LAUNCH_TABLE[(256, 6)]

    def test_mse_unknown_shape_falls_back_to_default(self, monkeypatch):
        for var in ("GENESIS_P40_BLOCK_H", "GENESIS_P40_BLOCK_KV",
                    "GENESIS_P40_NUM_WARPS", "GENESIS_P40_NUM_STAGES"):
            monkeypatch.delenv(var, raising=False)
        from vllm._genesis.kernels import tq_grouped_decode as k
        assert k.resolve_launch_config(
            head_dim=128, kv_group_size=4, key_fp8=False,
        ) == k._MSE_LAUNCH_DEFAULT

    def test_env_overrides_apply(self, monkeypatch):
        monkeypatch.setenv("GENESIS_P40_BLOCK_KV", "64")
        monkeypatch.setenv("GENESIS_P40_NUM_WARPS", "8")
        monkeypatch.setenv("GENESIS_P40_NUM_STAGES", "3")
        from vllm._genesis.kernels import tq_grouped_decode as k
        bh, bkv, nw, ns = k.resolve_launch_config(
            head_dim=256, kv_group_size=6, key_fp8=False,
        )
        assert (bkv, nw, ns) == (64, 8, 3)

    def test_invalid_env_values_ignored(self, monkeypatch):
        # 12 not in BLOCK_KV whitelist; 'x' not an int; 9 > stages cap
        monkeypatch.setenv("GENESIS_P40_BLOCK_KV", "12")
        monkeypatch.setenv("GENESIS_P40_NUM_WARPS", "x")
        monkeypatch.setenv("GENESIS_P40_NUM_STAGES", "9")
        from vllm._genesis.kernels import tq_grouped_decode as k
        assert k.resolve_launch_config(
            head_dim=256, kv_group_size=6, key_fp8=False,
        ) == k._MSE_LAUNCH_TABLE[(256, 6)]
