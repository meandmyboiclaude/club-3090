...
class TurboQuantConfig:
    ...
    def get_boundary_skip_layers(
        ...
        # CLUB3090 GATE-2 WORKAROUND (2026-05-11) — return [] so NO layers
        # are skipped. Forces all 60 Gemma 4 layers through TURBOQUANT
        # backend, bypassing the missing per-layer routing (Gate 2). Cost:
        # boundary precision loss documented in the original docstring below.
        # Quality cost is real: upstream notes "GSM8K drops ~30 points on
        # Qwen3-4B" without boundary protection on aggressive presets.
        # Drop this patch when upstream adds per-layer backend routing.
        import logging as _logging
        _logging.getLogger(__name__).warning(
            "[club3090-gate2] get_boundary_skip_layers forced to [] — boundary precision off"
        )
        return []

    @staticmethod
    def _orig_get_boundary_skip_layers(
        model_config: ModelConfig,
        n: int = 2,
    ) -> list[str]:
