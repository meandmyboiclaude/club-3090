...
class TurboQuantAttentionImpl(AttentionImpl["TurboQuantMetadata"]):
    ...
    def forward(
        ...
        if not attn_metadata.is_prefill:
            ...
            # club3090 cudagraph-capture guard: .tolist() forces a GPU→CPU
            # sync which raises during cudagraph capture. Fall back to the
            # already-known batch max_seq_len when capture is active —
            # safe because the captured graph is a no-op warmup anyway.
            if torch.cuda.is_current_stream_capturing():
                prefill_max_seq = int(attn_metadata.max_seq_len)
            else:
    ...
    def _prefill_attention(
        ...
        # club3090 cudagraph-capture guard: .tolist() forces a GPU→CPU sync
        # which raises during cudagraph capture. The Python loop below
        # can't run inside cudagraphs anyway — during capture, return all
        # zeros so the captured graph is a no-op warmup. Real runtime
        # invocations bypass this guard and behave normally.
        if torch.cuda.is_current_stream_capturing():
            num_reqs_int = int(query_start_loc.shape[0]) - 1
            qsl = [0] * (num_reqs_int + 1)
            seq_lens_list = [0] * num_reqs_int
        else:
