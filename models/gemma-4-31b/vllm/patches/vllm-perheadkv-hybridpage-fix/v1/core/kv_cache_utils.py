...
def unify_kv_cache_spec_page_size(
    ...
    size by increasing the block size of layers with smaller page size. If a
    smaller page size does not cleanly divide the largest page size, pad that
    layer to the largest page size when the spec supports page_size_padded.
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        if layer_spec.page_size_bytes == target_page_size:
            ...
            elif hasattr(layer_spec, "page_size_padded"):
                new_spec = replace(layer_spec, page_size_padded=target_page_size)
            else:
                raise NotImplementedError(
                    f"Layer {layer_name} (type {type(layer_spec).__name__}) "
                    "has incompatible page size and does not support padding."
                )
            assert new_spec.page_size_bytes == target_page_size
            new_kv_cache_spec[layer_name] = new_spec
    assert is_kv_cache_page_size_uniform(new_kv_cache_spec)
